...
import {
  ...
  updateItem,
  deleteItem,
} from "../services/sub_district";
import SubdistrictModal from "../components/SubdistrictModal";
...
function Subdistrict() {
  ...
  const transformData = (obj: typeOfRawRow): typeOfMuiRow => {
    if (!obj || !obj.boundary || !obj.district) {
    ...
    return {
      ...
      district_id: obj.district.geo_id,
    };
  };
  ...
  interface typeOfUpdatedsubdistrict {
    geo_id: number;
    geo_code: string;
    name_en: string;
    name_th: string;
  ...
  const handleSubmit = async (updatedCountry: typeOfUpdatedsubdistrict) => {
    // เเสดง loading
    setLoading(true);
    try {
  ...
  return (
    <>
      <AppBarCustom title="Sub District ตำบล" />
      {loading ? (
